...
public:
	...
	ThreeVector(const ThreeVector<bType> &copyMeVector){ mx=copyMeVector.x(); my=copyMeVector.y(); mz=copyMeVector.z();} //copy constructor
	...
	//Assignment Operator
	template <typename bType>
	ThreeVector operator=(const ThreeVector<bType>& assignFromVector){ 
	...
	// +=, plus operator
	template <typename bType>
	ThreeVector& operator+=(const ThreeVector<bType>& addMeVector){
	...
	template <typename bType>
	ThreeVector& operator-=(const ThreeVector<bType>& subtractMeVector){
				 mx-=subtractMeVector.x(); my-=subtractMeVector.y(); mz-=subtractMeVector.z();
				 return *this;}

	// *=, multiplication operator
	template <typename bType>
	ThreeVector& operator*=(const bType& scalarFactor){
	...
	template <typename bType>
	ThreeVector& operator/=(const bType& scalarDivisor){
	...
	//cross-product
	template <typename bType>
	ThreeVector cross(const ThreeVector<bType>& crossVec){
...
bool operator==(const ThreeVector<aType>& vA,const ThreeVector<bType>& vB){ return (vA.x()==vB.x())&&(vA.y()==vB.y())&&(vA.z()==vB.z()); }
template <class aType, class bType>
bool operator!=(const ThreeVector<aType>& vA,const ThreeVector<bType>& vB){ return !(vA==vB); }
...
template <class aType, class bType>
ThreeVector<aType> operator+(const ThreeVector<aType>&  leftSummand, const ThreeVector<bType>& rightSummand){
...
// - operator
template <class aType, class bType>
ThreeVector<aType> operator-(ThreeVector<aType> minuend, const ThreeVector<bType>& subtrahend){
	minuend-=subtrahend;
	return minuend;}
...
template <class aType, class bType>
ThreeVector<aType> operator*(ThreeVector<aType> leftVectorFactor, const bType& rightScalarFactor){
	leftVectorFactor*=rightScalarFactor;
	return leftVectorFactor;}

// * operator -- scalar on the left
template <class aType, class bType>
ThreeVector<aType> operator*(const bType& leftScalarFactor,ThreeVector<aType> rightVectorFactor){
...
template <class aType, class bType>
ThreeVector<aType> operator/(ThreeVector<aType> leftVectorDividend, const bType& rightScalarDivisor){
...
// dot-product
template <class aType, class bType>
aType operator*(const ThreeVector<aType>& U, const ThreeVector<bType>& V){
...
template <class aType, class bType>
void productVectorMatrix(ThreeVector<aType>& aVector,const bType rotationMatrix[3][3]){
