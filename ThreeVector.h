...
public:
	...
	//riend std::ostream& operator<<(std::ostream& os, const ThreeVector<bType>& vOut);
	...
	//// Vector Operations
	ThreeVector cross(const ThreeVector& crossVec); //cross-product

	ThreeVector& rotateX(const aType& phi); //rotate the vector by angle phi (in radians) around the x axis
	ThreeVector& rotateY(const aType& phi); //rotate the vector by angle phi (in radians) around the y axis
	ThreeVector& rotateZ(const aType& phi); //rotate the vector by angle phi (in radians) around the z axis

	ThreeVector unit()const;

	//// auxiliary functions
	// get the 3x3 rotation for arbitrary rotation around the x,y,z axis. xPhi is the angle in radiation of which we rotate around the x-Axis, yPhi, and zPhi analog. 
